...
export default function HeroCarousel() {
  ...
  return (
    <div className="card p-3 relative overflow-hidden">
      ...
      <div className="relative w-full h-[340px] rounded-xl overflow-hidden bg-[var(--paper-2)]">
        {slides.map((s, idx) => (
          <a key={s.src} href={s.href} className={`absolute inset-0 transition-all duration-700 ${idx===i ? "opacity-100 scale-100" : "opacity-0 scale-[1.03]"}`}>
            <img src={s.src} alt={s.title} className="w-full h-full object-cover" loading={idx===0?"eager":"lazy"} />
          </a>
        ))}
